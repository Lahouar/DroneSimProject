...
def get_command(sensor_data, camera_data, dt):
    ...
    global frames, poses, control_command, right_scan, left_scan, center_aligned, go_forward, inside_door, door_pos, turn_counter, initial_pos, grid_size, bounds, step, offset_correct, fast_laps, stupid_path, path
    ...
    if door_pos is not None:
        if len(door_pos) == 5:
            if distance(sensor_data, door_pos[-1]) > 0.5:
    ...
    if door_pos is not None:
        if len(door_pos) == 6:
            ...
            corr_pos = [intial_pos[0], initial_pos[1], initial_pos[2], initial_pos[3]]
